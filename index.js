...
function indekseGoreFenomen(degisken, index) {
  console.log(
    `${index} . indekste bulunan fenomen: ${degisken[index].profile}`
  );
}
...
function profilListesi(sayac) {
  const dizi = sayac;
  const render = dizi.map((item) => {
    return item.profile;
  });
  return render;
}
...
function fenomenSil(sayac, index) {
  const dizi = sayac;
  const render = dizi.filter((item, sayac) => {
    return sayac !== index;
  });
  return render;
}
...
function fenomenEkle(deger, number, profile, followers, posts, platform) {
  const sayac = deger;
  const nesne = {
    number,
    profile,
...
function enFenomenler(sayac) {
  const dizi = sayac;
  const render = dizi.filter((item) => item.followers > 100000000);
  const filtrender = render.map((item) => item.profile);
  return filtrender;
}
...
function fenomenGonderimSayisi(sayac, degisken) {
  const dizi = sayac;
  const dizimap = dizi.map((item) => item);
  const dizifilter = dizimap.filter((item) => item.profile === degisken);
  const posts = dizifilter.map((item) => item.posts);
  console.log(posts);
  return posts;
}
...
function platformaGoreCokGonderiYapanFenomen(degisken, platform) {
  let deger = 0;
  const sayac = degisken;
  const render = sayac.filter((item) => platform === item.platform);
  const map = render.map((item) => {
    if (item.posts > deger) {
      deger = item.posts;
    }
  });
  const filter = render.filter((item) => item.posts === deger);

  const kisi = filter.map((item) => item.profile);
  console.log(kisi);
  return kisi;
}
